...
public class LocalDiskRepositoryManager implements GitRepositoryManager {
    ...
    private Repository openRepository(File path, Project.NameKey name)
        ...
        if (!names.contains(name)) {
            ...
            if (!name.get().endsWith(Constants.DOT_GIT_EXT)) {
                if (RepositoryCache.FileKey.resolve(gitDir, FS.DETECTED) != null) {
                ...
                if (RepositoryCache.FileKey.isGitRepository(new File(directory, Constants.DOT_GIT),
                        FS.DETECTED)) {
                    onCreateProject(name);
                } else if (RepositoryCache.FileKey.isGitRepository(new File(directory.getParentFile(),
        ...
        final RepositoryCache.FileKey loc = RepositoryCache.FileKey.lenient(gitDir, FS.DETECTED);
        try {
    ...
    private Repository createRepository(File path, Project.NameKey name)
        ...
        File dir = RepositoryCache.FileKey.resolve(new File(path, name.get()), FS.DETECTED);
        ...
        if (dir != null) {
            ...
            loc = RepositoryCache.FileKey.exact(new File(path, n), FS.DETECTED);
        }
    ...
    private void scanProjects(final File dir, final String prefix,
        ...
        for (File f : ls) {
            ...
            } else if (RepositoryCache.FileKey.isGitRepository(f, FS.DETECTED)) {
